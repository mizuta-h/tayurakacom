...
function add_marketing_dp_default_options( $dp_default_options ) {
	...
	for ( $i = 1; $i<= 3; $i++ ) {
		...
		$dp_default_options['cta_type3_catch_bg_color' . $i] = "#ffffff";
		...
		$dp_default_options['footer_cta_type1_btn_bg_color' . $i] = '#bdbd07';
		$dp_default_options['footer_cta_type1_btn_bg_color_hover' . $i] = '#ecec00';
		...
		$dp_default_options['footer_cta_type2_btn_bg_color' . $i] = '#bdbd07';
		$dp_default_options['footer_cta_type2_btn_bg_color_hover' . $i] = '#ecec00';
  ...
  // ネイティブ広告
	for ( $i = 1; $i <= 7; $i++ ) {
		...
		$dp_default_options['pr_banner_label_bg_color'.$i] = '#d9d900';
	}

	// モーダルCTA トップページ設定
	$dp_default_options['show_modal_cta_front'] = 0;

	// モーダルCTA 下層ページ設定
	$dp_default_options['show_modal_cta_sub_same_front'] = 1;

	// モーダルCTA 下層ページ個別表示設定
	$dp_default_options['show_modal_cta_sub_archive_post'] = 0;
	$dp_default_options['show_modal_cta_sub_single_post'] = 0;
	$dp_default_options['show_modal_cta_sub_single_page'] = 0;
	$dp_default_options['show_modal_cta_sub_news_archive_post'] = 0;
	$dp_default_options['show_modal_cta_sub_news_single_post'] = 0;

	// モーダルCTA トップページ・下層ページ設定
	foreach ( array( 'front', 'sub' ) as $front_or_sub ) {
		$dp_default_options['modal_cta_' . $front_or_sub . '_type'] = 'type1';
		$dp_default_options['modal_cta_' . $front_or_sub . '_type1_overlay_color'] = '#004c66';
		$dp_default_options['modal_cta_' . $front_or_sub . '_type1_overlay_opacity'] = 0.4;
		$dp_default_options['modal_cta_' . $front_or_sub . '_image'] = 0;
		$dp_default_options['modal_cta_' . $front_or_sub . '_image_url'] = '';
		$dp_default_options['modal_cta_' . $front_or_sub . '_image_target_blank'] = 0;
		$dp_default_options['display_modal_cta_' . $front_or_sub . '_catch'] = 0;
		$dp_default_options['modal_cta_' . $front_or_sub . '_catch'] = '';
		$dp_default_options['modal_cta_' . $front_or_sub . '_catch_font_size'] = 40;
		$dp_default_options['modal_cta_' . $front_or_sub . '_catch_font_size_mobile'] = 28;
		$dp_default_options['modal_cta_' . $front_or_sub . '_catch_color'] = '#ffffff';
		$dp_default_options['modal_cta_' . $front_or_sub . '_catch_font_type'] = 'type2';
		$dp_default_options['display_modal_cta_' . $front_or_sub . '_desc'] = 0;
		$dp_default_options['modal_cta_' . $front_or_sub . '_desc'] = '';
		$dp_default_options['modal_cta_' . $front_or_sub . '_desc_font_size'] = 16;
		$dp_default_options['modal_cta_' . $front_or_sub . '_desc_font_size_mobile'] = 14;
		$dp_default_options['modal_cta_' . $front_or_sub . '_desc_color'] = '#ffffff';
		$dp_default_options['modal_cta_' . $front_or_sub . '_desc_font_type'] = 'type2';
		$dp_default_options['modal_cta_' . $front_or_sub . '_editor'] = '';
		$dp_default_options['modal_cta_' . $front_or_sub . '_editor_bg_color'] = '#ffffff';
		$dp_default_options['modal_cta_' . $front_or_sub . '_only_once'] = 0;
	}

	// ミニCTA
	$dp_default_options['show_mini_cta'] = 0;
	$dp_default_options['mini_cta_catch'] = '';
	$dp_default_options['mini_cta_catch_font_color'] = '#33ccff';
	$dp_default_options['mini_cta_desc'] = '';
	$dp_default_options['mini_cta_button_label'] = '';
	$dp_default_options['mini_cta_button_url'] = '';
	$dp_default_options['mini_cta_button_target_blank'] = 0;
	$dp_default_options['mini_cta_button_font_color_hover'] = '#33ccff';
...
<div id="tab-content-marketing" class="tab-content">
      ...
      <div class="sub_box_content">
       ...
       <div class="cta-type3-content cta-content <?php if ( 'type3' !== $options['cta_type' . $i] ) { echo 'u-hidden'; } ?>">
        ...
        <ul class="option_list">
         ...
         <li class="cf"><span class="label"><?php _e('Background color', 'tcd-w'); ?></span><input type="text" name="dp_options[cta_type3_catch_bg_color<?php echo $i; ?>]" value="<?php echo esc_attr( $options['cta_type3_catch_bg_color'.$i] ); ?>" data-default-color="#ffffff" class="c-color-picker"></li>
        </ul>
        <h6 class="theme_option_headline2"><?php _e( 'Link setting', 'tcd-w' ); ?></h6>
        <ul class="option_list">
       ...
       <div class="sub_box_content">
        ...
        <div class="cta-type3-content cta-content <?php if ( 'type3' !== $options['footer_cta_type' . $i] ) { echo 'u-hidden'; } ?>">
         ...
         <ul class="option_list">
          ...
          <li class="cf"><span class="label"><?php _e('Transparency of overlay', 'tcd-w'); ?></span><input class="small-text" type="number" max="1" min="0" step="0.1" name="dp_options[footer_cta_type3_overlay_opacity<?php echo $i; ?>]" value="<?php echo esc_attr( $options['footer_cta_type3_overlay_opacity' . $i] ); ?>"></li>
          <li class="cf">
   ...
   <div class="theme_option_field cf theme_option_field_ac">
    ...
    <div class="theme_option_field_ac_content">
     ...
     <div class="theme_option_message2">
      <p><?php _e( 'Native ads will be displayed in front page and blog archive page.', 'tcd-w' ); ?></p>
     </div>

     <?php for($i = 1; $i <= 7; $i++) : ?>
     <div class="sub_box cf"<?php if($i == 1){ echo ' style="margin-top:20px;"'; }; ?>>
      <h3 class="theme_option_subbox_headline"><?php printf(__('Ads%s setting and changes', 'tcd-w'), $i); ?></h3>
      <div class="sub_box_content">
       ...
       <div style="<?php if($options['show_pr_banner'.$i] == 1) { echo 'display:block;'; } else { echo 'display:none;'; }; ?>">
        ...
        <p><?php printf(__('Recommend image size. Width:%1$spx, Height:%2$spx.', 'tcd-w'), '794', '540'); ?></p>
        <div class="image_box cf">
        ...
        <div style="<?php if($options['show_pr_banner_label'.$i] == 1) { echo 'display:block;'; } else { echo 'display:none;'; }; ?>">
         <ul class="option_list" style="border-top:1px dotted #ccc; padding-top:12px;">
          ...
          <li class="cf">
           <span class="label"><?php _e('Border color', 'tcd-w'); ?></span>
           <input class="c-color-picker" type="text" name="dp_options[pr_banner_label_bg_color<?php echo $i; ?>]" value="<?php echo esc_attr( $options['pr_banner_label_bg_color'.$i] ); ?>" data-default-color="#d9d900">
          </li>
         </ul>
        </div>
       </div>
	...
	<?php // Modal CTA ?>
	<div class="theme_option_field cf theme_option_field_ac">
		<h3 class="theme_option_headline"><?php _e( 'Modal CTA', 'tcd-w' ); ?></h3>
		<div class="theme_option_field_ac_content">
			<div class="theme_option_message2">
				<p><?php _e( 'You can set up Modal CTA.', 'tcd-w' ); ?></p>
			</div>

			<?php // Modal CTA front page ?>
			<div class="sub_box">
				<h4 class="theme_option_subbox_headline"><?php _e( 'Modal CTA setting for front page', 'tcd-w' ); ?></h4>
				<div class="sub_box_content">
					...
					<p><label><input type="checkbox" name="dp_options[show_modal_cta_front]" value="1" data-toggle=".toggle-modal_cta_front" <?php checked( $options['show_modal_cta_front'], 1 ); ?>> <?php _e( 'Show Modal CTA on front page', 'tcd-w' ); ?></label></p>
					<div class="toggle-modal_cta_front"<?php if ( ! $options['show_modal_cta_front'] && ! $options['show_modal_cta_sub_same_front'] ) echo ' style="display: none;"'; ?>>
<?php
	add_modal_cta_tab_panel_sub( $options, 'front' );
?>
					</div>

					<ul class="button_list cf">
						<li><input type="submit" class="button-ml ajax_button" value="<?php _e( 'Save Changes', 'tcd-w' ); ?>" /></li>
						<li><a class="close_sub_box button-ml" href="javascript:void(0);"><?php _e( 'Close', 'tcd-w' ); ?></a></li>
					</ul>
				</div>
			</div>

			<?php // Modal CTA sub page ?>
			<div class="sub_box">
				<h4 class="theme_option_subbox_headline"><?php _e( 'Modal CTA setting for sub page', 'tcd-w' ); ?></h4>
				<div class="sub_box_content">
					<div class="theme_option_message2" style="margin-top: 10px;">
						<p><?php _e( 'Set the modal CTA to be displayed on the sub page.', 'tcd-w' ); ?></p>
					</div>
					<p><label><input type="checkbox" name="dp_options[show_modal_cta_sub_same_front]" value="1" data-toggle-reverse=".toggle-modal_cta_sub" data-toggle-show=".toggle-modal_cta_front" <?php checked( $options['show_modal_cta_sub_same_front'], 1 ); ?>> <?php _e( 'Same as front page settings', 'tcd-w' ); ?></label></p>
					<div class="toggle-modal_cta_sub" <?php if ( $options['show_modal_cta_sub_same_front'] ) echo ' style="display: none;"'; ?>>
<?php
	add_modal_cta_tab_panel_sub( $options, 'sub' );
?>
					</div>
					<div class="show_modal_cta_sub_checkboxes checkboxes">
						<h4 class="theme_option_headline2"><?php _e( 'Display settings', 'tcd-w' ); ?></h4>
						<ul>
							<li><label><input name="dp_options[show_modal_cta_sub_archive_post]" type="checkbox" value="1" <?php checked( 1, $options['show_modal_cta_sub_archive_post'] ); ?>> <?php _e( 'Display Modal CTA on post archive page', 'tcd-w' ); ?></label></li>
							<li><label><input name="dp_options[show_modal_cta_sub_single_post]" type="checkbox" value="1" <?php checked( 1, $options['show_modal_cta_sub_single_post'] ); ?>> <?php _e( 'Display Modal CTA on post single page', 'tcd-w' ); ?></label></li>
							<li><label><input name="dp_options[show_modal_cta_sub_news_archive_post]" type="checkbox" value="1" <?php checked( 1, $options['show_modal_cta_sub_news_archive_post'] ); ?>> <?php echo sprintf(__( 'Display Modal CTA on %s archive page', 'tcd-w' ), $news_label); ?></label></li>
							<li><label><input name="dp_options[show_modal_cta_sub_news_single_post]" type="checkbox" value="1" <?php checked( 1, $options['show_modal_cta_sub_news_single_post'] ); ?>> <?php echo sprintf(__( 'Display Modal CTA on %s single page', 'tcd-w' ), $news_label); ?></label></li>
							<li><label><input name="dp_options[show_modal_cta_sub_single_page]" type="checkbox" value="1" <?php checked( 1, $options['show_modal_cta_sub_single_page'] ); ?>> <?php _e( 'Display Modal CTA on page', 'tcd-w' ); ?></label></li>
						</ul>
					</div>
			...
			<ul class="button_list cf">
				<li><input type="submit" class="button-ml ajax_button" value="<?php _e( 'Save Changes', 'tcd-w' ); ?>" /></li>
				<li><a class="close_ac_content button-ml" href="javascript:void(0);"><?php _e( 'Close', 'tcd-w' ); ?></a></li>
			</ul>
		</div>
	</div>
	...
	<?php // ミニCTA ?>
	<div class="theme_option_field cf theme_option_field_ac">
		<h3 class="theme_option_headline"><?php _e( 'Popup CTA', 'tcd-w' ); ?></h3>
		<div class="theme_option_field_ac_content">
			<div class="theme_option_message2">
				<p><?php _e( 'You can set up Popup CTA which is displayed at the bottom right of the screen on scroll.<br>If click the close button in Popup CTA, hide the Popup CTA until browser closes.', 'tcd-w' ); ?><br><?php _e('While popup CTA is displayed, fotter CTA is not displayed.', 'tcd-w'); ?></p>
			</div>
			<p><label><input type="checkbox" name="dp_options[show_mini_cta]" value="1" data-toggle-show=".mini_cta" <?php checked( $options['show_mini_cta'], 1 ); ?>> <?php _e( 'Show Popup CTA', 'tcd-w' ); ?></label></p>
			<div class="toggle-mini_cta">
				<h4 class="theme_option_headline2"><?php _e( 'Catchphrase', 'tcd-w' ); ?></h4>
				<textarea class="large-text" rows="2" name="dp_options[mini_cta_catch]"><?php echo esc_textarea( $options['mini_cta_catch'] ); ?></textarea>
				<table class="theme_option_table">
					<tr>
						<td><?php _e( 'Font color', 'tcd-w' ); ?></td>
						<td><input type="text" name="dp_options[mini_cta_catch_font_color]" value="<?php echo esc_attr( $options['mini_cta_catch_font_color'] ); ?>" data-default-color="<?php echo esc_attr( $dp_default_options['mini_cta_catch_font_color'] ); ?>" class="c-color-picker"></td>
					</tr>
				</table>
				<h4 class="theme_option_headline2"><?php _e( 'Description', 'tcd-w' ); ?></h4>
				<textarea class="large-text" rows="4" name="dp_options[mini_cta_desc]"><?php echo esc_textarea( $options['mini_cta_desc'] ); ?></textarea>
				<h4 class="theme_option_headline2"><?php _e( 'Link button', 'tcd-w' ); ?></h4>
				<table class="theme_option_table c-color-picker--static">
					<tr>
						<td><?php _e( 'Button label', 'tcd-w' ); ?></td>
						<td><input type="text" class="large-text" name="dp_options[mini_cta_button_label]" value="<?php echo esc_attr( $options['mini_cta_button_label'] ); ?>">
					</tr>
					<tr>
						<td><?php _e( 'Button link URL', 'tcd-w' ); ?></td>
						<td>
							<input type="url" class="large-text" name="dp_options[mini_cta_button_url]" value="<?php echo esc_attr( $options['mini_cta_button_url'] ); ?>">
							<label><input type="checkbox" name="dp_options[mini_cta_button_target_blank]" value="1" <?php checked( 1, $options['mini_cta_button_target_blank'] ); ?>> <?php _e( 'Open with new window', 'tcd-w' ); ?></label>
						</td>
					</tr>
					<tr>
						<td><?php _e('Font color on mouseover', 'tcd-w'); ?></td>
						<td><input type="text" name="dp_options[mini_cta_button_font_color_hover]" value="<?php echo esc_attr( $options['mini_cta_button_font_color_hover'] ); ?>" data-default-color="<?php echo esc_attr( $dp_default_options['mini_cta_button_font_color_hover'] ); ?>" class="c-color-picker"></td>
					</tr>
				</table>
			</div>
...
function add_marketing_theme_options_validate( $input ) {
  ...
  global $dp_default_options, $cta_type_options, $cta_type3_layout_options, $cta_display_options, $footer_cta_type_options, $modal_cta_type_options;
    ...
    // CTA type3
		...
		$input['cta_type3_catch_bg_color' . $i] = wp_filter_nohtml_kses( $input['cta_type3_catch_bg_color' . $i] );
  ...
  for ( $i = 1; $i <= 7; $i++ ) {
	...
	$input['show_modal_cta_sub_news_archive_post'] = ! empty( $input['show_modal_cta_sub_news_archive_post'] ) ? 1 : 0;
	$input['show_modal_cta_sub_news_single_post'] = ! empty( $input['show_modal_cta_sub_news_single_post'] ) ? 1 : 0;

	// モーダルCTA トップページ・下層ページ設定
	foreach ( array( 'front', 'sub' ) as $front_or_sub ) {
		if ( ! isset( $input['modal_cta_' . $front_or_sub . '_type'] ) || ! array_key_exists( $input['modal_cta_' . $front_or_sub . '_type'], $modal_cta_type_options ) )
			$input['modal_cta_' . $front_or_sub . '_type'] = $dp_default_options['modal_cta_' . $front_or_sub . '_type'];
		$input['modal_cta_' . $front_or_sub . '_type1_overlay_color'] = sanitize_hex_color( $input['modal_cta_' . $front_or_sub . '_type1_overlay_color'] );
		$input['modal_cta_' . $front_or_sub . '_type1_overlay_opacity'] = sanitize_text_field( $input['modal_cta_' . $front_or_sub . '_type1_overlay_opacity'] );
		$input['display_modal_cta_' . $front_or_sub . '_image'] = ! empty( $input['display_modal_cta_' . $front_or_sub . '_image'] ) ? 1 : 0;
		$input['modal_cta_' . $front_or_sub . '_image'] = absint( $input['modal_cta_' . $front_or_sub . '_image'] );
		$input['modal_cta_' . $front_or_sub . '_image_url'] = sanitize_text_field( $input['modal_cta_' . $front_or_sub . '_image_url'] );
		$input['modal_cta_' . $front_or_sub . '_image_target_blank'] = ! empty( $input['modal_cta_' . $front_or_sub . '_image_target_blank'] ) ? 1 : 0;
		$input['display_modal_cta_' . $front_or_sub . '_catch'] = ! empty( $input['display_modal_cta_' . $front_or_sub . '_catch'] ) ? 1 : 0;
		$input['modal_cta_' . $front_or_sub . '_catch'] = sanitize_textarea_field( $input['modal_cta_' . $front_or_sub . '_catch'] );
		$input['modal_cta_' . $front_or_sub . '_catch_font_size'] = absint( $input['modal_cta_' . $front_or_sub . '_catch_font_size'] );
		$input['modal_cta_' . $front_or_sub . '_catch_font_size_mobile'] = absint( $input['modal_cta_' . $front_or_sub . '_catch_font_size_mobile'] );
		$input['modal_cta_' . $front_or_sub . '_catch_color'] = sanitize_hex_color( $input['modal_cta_' . $front_or_sub . '_catch_color'] );
		if ( ! isset( $input['modal_cta_' . $front_or_sub . '_catch_font_type'] ) || ! array_key_exists( $input['modal_cta_' . $front_or_sub . '_catch_font_type'], $font_type_options ) )
			$input['modal_cta_' . $front_or_sub . '_catch_font_type'] = $dp_default_options['modal_cta_' . $front_or_sub . '_catch_font_type'];
		$input['display_modal_cta_' . $front_or_sub . '_desc'] = ! empty( $input['display_modal_cta_' . $front_or_sub . '_desc'] ) ? 1 : 0;
		$input['modal_cta_' . $front_or_sub . '_desc'] = sanitize_textarea_field( $input['modal_cta_' . $front_or_sub . '_desc'] );
		$input['modal_cta_' . $front_or_sub . '_desc_font_size'] = absint( $input['modal_cta_' . $front_or_sub . '_desc_font_size'] );
		$input['modal_cta_' . $front_or_sub . '_desc_font_size_mobile'] = absint( $input['modal_cta_' . $front_or_sub . '_desc_font_size_mobile'] );
		$input['modal_cta_' . $front_or_sub . '_desc_color'] = sanitize_hex_color( $input['modal_cta_' . $front_or_sub . '_desc_color'] );
		if ( ! isset( $input['modal_cta_' . $front_or_sub . '_desc_font_type'] ) || ! array_key_exists( $input['modal_cta_' . $front_or_sub . '_desc_font_type'], $font_type_options ) )
			$input['modal_cta_' . $front_or_sub . '_desc_font_type'] = $dp_default_options['modal_cta_' . $front_or_sub . '_desc_font_type'];
		$input['modal_cta_' . $front_or_sub . '_editor'] = $input['modal_cta_' . $front_or_sub . '_editor'];
		$input['modal_cta_' . $front_or_sub . '_editor_bg_color'] = sanitize_hex_color( $input['modal_cta_' . $front_or_sub . '_editor_bg_color'] );
		$input['modal_cta_' . $front_or_sub . '_only_once'] = ! empty( $input['modal_cta_' . $front_or_sub . '_only_once'] ) ? 1 : 0;
	}

	// ミニCTA
	$input['show_mini_cta'] = ! empty( $input['show_mini_cta'] ) ? 1 : 0;
	...
	$input['mini_cta_button_target_blank'] = ! empty( $input['mini_cta_button_target_blank'] ) ? 1 : 0;
	$input['mini_cta_button_font_color_hover'] = sanitize_hex_color( $input['mini_cta_button_font_color_hover'] );
...
// モーダルCTA フロントページ・サブページ共通フォーム
function add_modal_cta_tab_panel_sub( $dp_options, $front_or_sub ) {
	global $dp_default_options, $modal_cta_type_options, $font_type_options, $text_align_options;
?>
						<h4 class="theme_option_headline2"><?php _e( 'Contents type', 'tcd-w' ); ?></h4>
						<div class="theme_option_message2"><?php _e( '<p>Banner template: Show image and text.<br>Wysiwyg editor: Create content freely as you like blog posts.</p>', 'tcd-w' ); ?></div>
						<ul>
							<?php foreach( $modal_cta_type_options as $option ) : ?>
							<li><label><input type="radio" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_type]" value="<?php echo esc_attr( $option['value'] ); ?>" data-toggle=".modal_cta_<?php echo $front_or_sub; ?>_type-<?php echo esc_attr( $option['value'] ); ?>" data-hide="[class*='modal_cta_<?php echo $front_or_sub; ?>_type-']" <?php checked( $option['value'], $dp_options['modal_cta_' . $front_or_sub . '_type'] ); ?>> <?php echo $option['label']; ?></label></li>
							<?php endforeach; ?>
						</ul>
						<div class="modal_cta_<?php echo $front_or_sub; ?>_type-type1"<?php if ( 'type1' !== $dp_options['modal_cta_' . $front_or_sub . '_type'] ) echo ' style="display: none;"'; ?>>
							<h4 class="theme_option_headline2"><?php _e( 'Image', 'tcd-w' ); ?></h4>
							<div class="theme_option_message2">
								<p><?php _e( 'Set banner image', 'tcd-w' ); ?><br><?php printf( __( 'Recommend image size. Width:%dpx, Height:%dpx', 'tcd-w' ), 750, 400 ); ?></p>
							</div>
							<div class="image_box cf">
								<div class="cf cf_media_field hide-if-no-js modal_cta_<?php echo $front_or_sub; ?>_image">
									<input class="cf_media_id" type="hidden" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_image'] ); ?>" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_image]" id="modal_cta_<?php echo $front_or_sub; ?>_image">
									<div class="preview_field"><?php if ( $dp_options['modal_cta_' . $front_or_sub . '_image'] ) { echo wp_get_attachment_image( $dp_options['modal_cta_' . $front_or_sub . '_image'], 'full' ); } ?></div>
									<div class="button_area">
										<input type="button" value="<?php _e( 'Select Image', 'tcd-w' ); ?>" class="cfmf-select-img button">
										<input type="button" value="<?php _e( 'Remove Image', 'tcd-w' ); ?>" class="cfmf-delete-img button <?php if ( ! $dp_options['modal_cta_' . $front_or_sub . '_image'] ) { echo 'hidden'; } ?>">
									</div>
								</div>
							</div>
							...
							<p><label><?php _e( 'Link URL', 'tcd-w' ); ?> <input type="text" class="regular-text" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_image_url]" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_image_url'] ); ?>"></label></p>
							<p><label><input type="checkbox" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_image_target_blank]" value="1" <?php checked( 1, $dp_options['modal_cta_' . $front_or_sub . '_image_target_blank'] ); ?>> <?php _e( 'Open with new window', 'tcd-w' ); ?></label></p>
							<h4 class="theme_option_headline2"><?php _e( 'Catchphrase settings', 'tcd-w' ); ?></h4>
							<div class="theme_option_message2">
								<p><?php _e( 'You can set the catchphrase on the banner image, and check \"use catchphrase\" to set detail options.', 'tcd-w' ); ?></p>
							</div>
							<p><label><input type="checkbox" name="dp_options[display_modal_cta_<?php echo $front_or_sub; ?>_catch]" value="1" data-toggle=".toggle-display_modal_cta_<?php echo $front_or_sub; ?>_catch" <?php checked( $dp_options['display_modal_cta_' . $front_or_sub . '_catch'], 1 ); ?>><?php _e( 'Display catchphrase', 'tcd-w' ); ?></label></p>
							<div class="toggle-display_modal_cta_<?php echo $front_or_sub; ?>_catch"<?php if ( ! $dp_options['display_modal_cta_' . $front_or_sub . '_catch'] ) echo ' style="display: none;"' ?>>
								<h5 class="theme_option_headline2"><?php _e( 'Catchphrase', 'tcd-w' ); ?></h4>
								<div class="theme_option_message2">
									<p><?php _e( 'Please set texts for catchphrase, font size, color and align.', 'tcd-w' ); ?></p>
								</div>
								<textarea class="large-text" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_catch]" rows="2"><?php echo esc_textarea( $dp_options['modal_cta_' . $front_or_sub . '_catch'] ); ?></textarea>
								<table class="theme_option_table">
									<tr>
										<td><?php _e( 'Font size', 'tcd-w' ); ?></td>
										<td><input type="number" class="small-text" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_catch_font_size]" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_catch_font_size'] ); ?>" min="0"> px</td>
									</tr>
									<tr>
										<td><?php _e( 'Font size for mobile', 'tcd-w' ); ?></td>
										<td><input type="number" class="small-text" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_catch_font_size_mobile]" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_catch_font_size_mobile'] ); ?>" min="0"> px</td>
									</tr>
									<tr>
										<td><?php _e( 'Font color', 'tcd-w' ); ?></td>
										<td><input type="text" class="c-color-picker" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_catch_color]" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_catch_color'] ); ?>" data-default-color="<?php echo esc_attr( $dp_default_options['modal_cta_' . $front_or_sub . '_catch_color'] ); ?>"></td>
									</tr>
									<tr>
										<td><label><?php _e( 'Font type', 'tcd-w' ); ?></label></td>
										<td>
											<select name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_catch_font_type]">
												<?php foreach ( $font_type_options as $option ) : ?>
												<option value="<?php echo esc_attr( $option['value'] ); ?>" <?php selected( $option['value'], $dp_options['modal_cta_' . $front_or_sub . '_catch_font_type'] ); ?>><?php echo esc_html( $option['label'] ); ?></option>
												<?php endforeach; ?>
											</select>
										</td>
									</tr>
							...
							<h4 class="theme_option_headline2"><?php _e( 'Description settings', 'tcd-w' ); ?></h4>
							<div class="theme_option_message2">
								<p><?php _e( 'You can set the description below the catchphrase, and check \"use description\" to set detail options.', 'tcd-w' ); ?></p>
							</div>
							<p><label><input type="checkbox" name="dp_options[display_modal_cta_<?php echo $front_or_sub; ?>_desc]" value="1" data-toggle=".toggle-display_modal_cta_<?php echo $front_or_sub; ?>_desc" <?php checked( $dp_options['display_modal_cta_' . $front_or_sub . '_desc'], 1 ); ?>><?php _e( 'Display description', 'tcd-w' ); ?></label></p>
							<div class="toggle-display_modal_cta_<?php echo $front_or_sub; ?>_desc"<?php if ( ! $dp_options['display_modal_cta_' . $front_or_sub . '_desc'] ) echo ' style="display: none;"' ?>>
								...
								<textarea class="large-text" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_desc]" rows="5"><?php echo esc_textarea( $dp_options['modal_cta_' . $front_or_sub . '_desc'] ); ?></textarea>
								<table class="theme_option_table">
									<tr>
										<td><?php _e( 'Font size', 'tcd-w' ); ?></td>
										<td><input type="number" class="small-text" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_desc_font_size]" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_desc_font_size'] ); ?>" min="0"> px</td>
									</tr>
									<tr>
										<td><?php _e( 'Font size for mobile', 'tcd-w' ); ?></td>
										<td><input type="number" class="small-text" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_desc_font_size_mobile]" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_desc_font_size_mobile'] ); ?>" min="0"> px</td>
									</tr>
									<tr>
										<td><?php _e( 'Font color', 'tcd-w' ); ?></td>
										<td><input type="text" class="c-color-picker" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_desc_color]" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_desc_color'] ); ?>" data-default-color="<?php echo esc_attr( $dp_default_options['modal_cta_' . $front_or_sub . '_desc_color'] ); ?>"></td>
									</tr>
									<tr>
										<td><label><?php _e( 'Font type', 'tcd-w' ); ?></label></td>
										<td>
											<select name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_desc_font_type]">
												<?php foreach ( $font_type_options as $option ) : ?>
												<option value="<?php echo esc_attr( $option['value'] ); ?>" <?php selected( $option['value'], $dp_options['modal_cta_' . $front_or_sub . '_desc_font_type'] ); ?>><?php echo esc_html( $option['label'] ); ?></option>
												<?php endforeach; ?>
											</select>
										</td>
									</tr>
							...
							<h4 class="theme_option_headline2"><?php _e( 'Text contents background color setting', 'tcd-w' ); ?></h4>
							<div class="theme_option_message2">
								<p><?php _e( 'Sets the background color of catchphrase, sub catchphrase, description.', 'tcd-w' ); ?></p>
							</div>
							<input class="c-color-picker" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_type1_overlay_color]" type="text" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_type1_overlay_color'] ); ?>" data-default-color="<?php echo esc_attr( $dp_default_options['modal_cta_' . $front_or_sub . '_type1_overlay_color'] ); ?>">
							<h4 class="theme_option_headline2"><?php _e( 'Contents background opacity', 'tcd-w' ); ?></h4>
							<div class="theme_option_message2">
								<p><?php printf( __( 'Please set the opacity. (0 - 1.0, e.g. %s)', 'tcd-w'), '0.4' ); ?></p>
							</div>
							<input type="number" class="small-text" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_type1_overlay_opacity]" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_type1_overlay_opacity'] ); ?>" min="0" max="1" step="0.1">
						</div>
						<div class="modal_cta_<?php echo $front_or_sub; ?>_type-type2"<?php if ( 'type2' !== $dp_options['modal_cta_' . $front_or_sub . '_type'] ) echo ' style="display: none;"'; ?>>
...
<?php
	wp_editor(
		$dp_options['modal_cta_' . $front_or_sub . '_editor'],
		'modal_cta_' . $front_or_sub . '_editor',
		array(
			'textarea_name' => 'dp_options[modal_cta_' . $front_or_sub . '_editor]',
			'textarea_rows' => 10
		)
	);
?>
							<h4 class="theme_option_headline2"><?php _e( 'Text contents background color setting', 'tcd-w' ); ?></h4>
							<div class="theme_option_message2">
								<p><?php _e( 'Sets the background color of wysiwyg editor.', 'tcd-w' ); ?></p>
							</div>
							<input class="c-color-picker" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_editor_bg_color]" type="text" value="<?php echo esc_attr( $dp_options['modal_cta_' . $front_or_sub . '_editor_bg_color'] ); ?>" data-default-color="<?php echo esc_attr( $dp_default_options['modal_cta_' . $front_or_sub . '_editor_bg_color'] ); ?>">
						</div>
						<h4 class="theme_option_headline2"><?php _e( 'Display only once settings', 'tcd-w' ); ?></h4>
						<div class="theme_option_message2">
							<p><?php _e( 'Please check if you want to display CTA only when accessing page for the first time.', 'tcd-w' ); ?></p>
						</div>
						<p><label><input type="checkbox" name="dp_options[modal_cta_<?php echo $front_or_sub; ?>_only_once]" value="1" <?php checked( $dp_options['modal_cta_' . $front_or_sub . '_only_once'], 1 ); ?>><?php _e( 'Display only once until browser closes', 'tcd-w' ); ?></label></p>
<?php
}
