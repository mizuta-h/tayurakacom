...
__( '"CODE.", the 90th theme from TCD, is a WordPress theme that allows you to create a cool web magazine. The design allows you to appeal to a wide variety of contents from all angles. In addition to categories, tags, and authors,  by the number of views as well.', 'tcd-code' );
...
function my_admin_scripts() {
  ...
  wp_enqueue_script('my_script', get_template_directory_uri().'/admin/js/my_script.js', '', '2.0.8', true);
...
function my_admin_styles() {
  ...
  wp_enqueue_style('my_admin_css', get_template_directory_uri() .'/admin/css/my_admin.css','','2.0.2');
  wp_enqueue_style('new_ui_css', get_template_directory_uri() .'/admin/css/new_ui.css','','1.0.3');
...
register_sidebar(array(
  ...
  'before_title' => '<div class="widget_headline"><span>',
  'after_title' => "</span></div>",
  'name' => __('Featured content', 'tcd-w'),
  'description' => __('This widget will be displayed in <a href="./admin.php?page=theme_options" target="_blank">front page content builder</a> "Tab contents" and single page featured content.', 'tcd-w'),
  'id' => 'featured_widget'
));
register_sidebar(array(
  'before_widget' => '<div class="widget_content clearfix %2$s" id="%1$s">'."\n",
  ...
  'before_title' => '<div class="widget_headline"><span>',
  'after_title' => "</span></div>",
  'name' => __('Featured content (smartphone)', 'tcd-w'),
  'description' => __('This widget will be replaced with normal widget when a user accesses the site by smartphone.', 'tcd-w'),
  'id' => 'featured_widget_mobile'
));
register_sidebar(array(
  'before_widget' => '<div class="widget_content clearfix %2$s" id="%1$s">'."\n",
  'after_widget' => "</div>\n",
...
require get_template_directory() . '/functions/page_author_list.php';
require get_template_directory() . '/functions/page_ranking_list.php';
...
function trim_excerpt($a) {
 ...
 if(has_excerpt()) {
   ...
   $trim_content = mb_substr($base_content, 0, $a,"utf-8");
   $count_word = mb_strlen($trim_content,"utf-8");
   if($count_word > $a-1){
...
if ( function_exists('add_theme_support') ) {
  ...
  add_image_size( 'size2', 794, 540, true ); // ブログアーカイブページ
  add_image_size( 'size3', 884, 600, true ); // 詳細ページ
}
...
function message_image_meta_box($content, $post_id, $thumbnail_id) {
  ...
  if ( $post->post_type == 'post' || $post->post_type == 'news') {
    $content .= '<p>' . sprintf(__('Recommend image size. Width:%1$spx, Height:%2$spx.', 'tcd-w'), '884', '600') . '</p>';
    return $content;
  }
  if ( $post->post_type == 'page') {
    $content .= '<p class="eyecatch_notice1">' . sprintf(__('Recommend image size. Width:%1$spx, Height:%2$spx.<br>This image will be used in search result and OGP tag.', 'tcd-w'),'1200','630') . '</p><p class="eyecatch_notice2">' . sprintf(__('Recommend image size. Width:%1$spx, Height:%2$spx.<br>This image will be displayed at the bottom of the title.', 'tcd-w'),'1200','630') . '</p>';
    return $content;
  }
  return $content;
}
...
if(function_exists('register_nav_menu')) {
  ...
  register_nav_menu( 'footer-menu-bottom', __( 'Footer menu (bottom of page)', 'tcd-w' ));
}
...
function tcd_body_classes($classes) {
    ...
    if(is_search() && ( !have_posts() || empty( get_search_query() ))) {
      $classes[] = 'no_results_page';
    }
    if( ($options['show_load_screen'] != 'type1') ) { $classes[] = 'load_screen_animation_' . esc_attr($options['load_screen_animation_type']); };
    ...
    if( is_front_page() ) {
      ...
      if(($display_header_content == 'show') && $stop_animation){
        $classes[] = 'stop_index_slider_animation';
      }
    }
    if( is_page() && get_post_meta($post->ID, 'page_hide_logo', true) ) { $classes[] = 'hide_header_logo'; };
    if( is_page() && get_post_meta($post->ID, 'page_hide_bread', true) ) { $classes[] = 'hide_bread'; };
    if( is_page() && get_post_meta($post->ID, 'page_hide_search', true) ) { $classes[] = 'hide_search'; };
    if( is_page() && get_post_meta($post->ID, 'hide_page_header', true) ) { $classes[] = 'hide_page_header'; };
    if( is_page() && get_post_meta($post->ID, 'page_hide_sidemenu', true) ) { $classes[] = 'hide_sidemenu'; };
    ...
    $classes[] = 'side_menu_' . esc_attr($options['side_menu_type']);
...
$options = get_design_plus_option();
$news_label = isset($options['news_label']) ? esc_html( $options['news_label'] ) : __( 'News', 'tcd-w' );
$news_slug = isset($options['news_slug']) ? sanitize_title( $options['news_slug'] ) : 'news';
$news_labels = array(
  'name' => $news_label,
...
function copy_title_url_script() {
  ...
  if ( (is_singular('post') && $options['single_blog_show_copy_top']) || (is_singular('news') && $options['single_news_show_copy_top']) ) {
...
function tcd_remove_meta_boxes() {
  ...
  if ( 'page' === $typenow && ! empty( $post->ID ) && 'page' === get_option('show_on_front') && in_array( $post->ID, array( get_option( 'page_on_front' ), get_option( 'page_for_posts' ) ) ) ) {
    ...
    remove_meta_box( 'author_list_meta_box', 'page', 'normal' );
    remove_meta_box( 'ranking_list_meta_box', 'page', 'normal' );
