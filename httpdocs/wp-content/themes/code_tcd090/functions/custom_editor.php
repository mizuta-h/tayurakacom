...
function tcd_add_quicktags() {
	...
	for ( $i = 1; $i <= 3; $i++ ) {
		...
		$custom_buttons[$i] = '<div class="q_button_wrap"><a href="#" class="q_custom_button ' . $custom_button_class . '" style="' . $custom_button_size . '"><span class="label">' . sprintf( __( 'Button %d', 'tcd-w' ), $i ) . '</span></a></div>';
	}
...
<?php
     // ボタン ----------------------------------
     for ( $i = 1; $i <= 3; $i++ ) {
       if ( 'type4' === $dp_options['qt_custom_button_animation_type' . $i] ) {
         $design_button_border_color = $dp_options['qt_custom_button_underline_color'.$i];
?>
