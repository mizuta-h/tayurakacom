...
?>
  ...
  <h4 class="headline"><?php _e( 'Show author profile at author list page', 'tcd-w' ); ?></h4>
  <div class="input_field">
   <input name="show_author_list" type="checkbox" value="1" <?php checked( $user->show_author_list, 1 ); ?>> <?php _e( 'Show', 'tcd-w'); ?>
  </div><!-- END input_field -->

  <h4 class="headline"><?php _e( 'Catchphrase', 'tcd-w' ); ?></h4>
  <div class="input_field">
   <input type="text" name="catch" value="<?php echo esc_attr( get_the_author_meta( 'catch', $user->ID ) ); ?>" class="regular-text" />
  </div><!-- END input_field -->
...
?>
  ...
  <div class="input_field">
   <p><?php printf(__('Recommend image size. Width:%1$spx, Height:%2$spx.', 'tcd-w'), '360', '360'); ?></p>
...
function update_tcdw_profile_fields( $user_id ) {
	...
	if ( user_can( $user_id, 'edit_posts' ) ) {
		$meta_keys = array(
			...
			'show_author_list' => $_POST['show_author_list'],
