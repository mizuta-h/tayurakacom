...
class post_slider_widget extends WP_Widget {
  ...
  function widget($args, $instance) {
    ...
    $show_ads = $instance['show_ads'];
    $banner_num = $instance['banner_num'];
...
<div class="post_slider">
 ...
 <article class="item">
  ...
  <div class="title_area">
   <?php
        if ($options['archive_carousel_show_category']){
 ...
 <?php
      // native ads ------------------
      if(!empty($banner_list) && $show_ads && ($post_count % $banner_num == 0) ){
 ...
 <article class="item ad_item">
  ...
  <div class="title_area">
   ...
   <?php }; ?>
   <div class="title"><a href="<?php if($options['pr_banner_url'.$banner_num]) { echo esc_url($options['pr_banner_url'.$banner_num]); }; ?>" <?php if($options['pr_banner_target'.$banner_num]){ echo 'target="_blank"'; }; ?>><span><?php echo esc_html($options['pr_banner_title'.$banner_num]); ?></span></a></div>
  </div>
 </article>
...
<?php
  ...
  function update($new_instance, $old_instance) {
    ...
    $instance['show_ads'] = $new_instance['show_ads'];
  ...
  function form($instance) {
    $defaults = array('title' => 'RECOMMEND', 'post_num' => 3, 'post_type' => 'recent_post', 'post_order' => 'rand', 'show_ads' => '', 'banner_num' => 3);
...
<div class="tcd_widget_content">
 ...
 <p class="displayment_checkbox"><label><input name="<?php echo $this->get_field_name('show_ads'); ?>" type="checkbox" value="1" <?php checked( $instance['show_ads'], 1 ); ?>><?php _e( 'Display native ads', 'tcd-w' ); ?></label></p>
 <div style="<?php if($instance['show_ads'] == 1) { echo 'display:block;'; } else { echo 'display:none;'; }; ?>">
  <div class="theme_option_message2">
   <p><?php _e('Native ads will be displayed randomly at intervals of the number of articles set below.', 'tcd-w'); ?></p>
  </div>
  <input class="hankaku" style="width:70px;" type="number" max="9999" min="1" step="1" name="<?php echo $this->get_field_name('banner_num'); ?>" value="<?php echo esc_attr( $instance['banner_num'] ); ?>">
 </div>
</div>
